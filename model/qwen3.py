...
class Qwen3LinearScalingRotaryEmbedding(Qwen3RotaryEmbedding):
    """Qwen3RotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""

    def __init__(self, *args, **kwargs):
        logger.warning_once(
            "`Qwen3LinearScalingRotaryEmbedding` is deprecated an will be removed in v4.46. Please use "
            "`Qwen3RotaryEmbedding`, which now also does linear scaling (simply pass the model config to __init__)."
        )
...
class Qwen3DynamicNTKScalingRotaryEmbedding(Qwen3RotaryEmbedding):
    """Qwen3RotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""

    def __init__(self, *args, **kwargs):
        logger.warning_once(
            "`Qwen3DynamicNTKScalingRotaryEmbedding` is deprecated an will be removed in v4.46. Please use "
            "`Qwen3RotaryEmbedding`, which now also does dynamic ntk scaling (simply pass the model config to "
            "__init__).")
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: Optional[int] = None):
        ...
        # Qwen3 uses QK-norm (RMSNorm on head_dim applied to q/k before RoPE)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Reshape and apply QK-norm before RoPE
        query_states = self.q_norm(query_states.view(bsz, q_len, self.num_heads,
                                                     self.head_dim)).transpose(1, 2)
        key_states = self.k_norm(key_states.view(bsz, q_len, self.num_key_value_heads,
                                                 self.head_dim)).transpose(1, 2)
...
class Qwen3FlashAttention2(Qwen3Attention):
    """
    Llama flash attention module. This module inherits from `Qwen3Attention` as the weights of the module stays
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        >>> model = Qwen3ForCausalLM.from_pretrained("meta-llama/Llama-2-7b-hf")
